...
\label{sec:verwenden}
Dieses Paket stellt die experimentellen Trennmuster für der Verwendung
mit \LaTeX\ und dem Paket \texttt{babel} zur Verfügung.  Es kann sowohl
mit traditionellen 8-Bit-fähigen \TeX-Varianten (\TeX\ und pdf\TeX) als
auch modernen 16-Bit-fähigen \TeX-Varianten (Lua\TeX\ und Xe\TeX)
verwendet werden.  Da \texttt{babel} in Verbindung mit Lua\TeX\ und
Xe\TeX\ für die Sprachbezeichner \texttt{german} bzw. \texttt{ngerman}
bereits standardmäßig die experimentellen Trennmuster lädt --
bereitgestellt durch das Paket \texttt{hyph-utf8}~--, liegt der
Hauptnutzen dieses Pakets allerdings in der Verwendung der
experimentellen Trennmuster mit traditionellen \TeX-Varianten.  Nutzer
von p\TeX%
\footnote{Eine in Japan populäre \TeX-Variante.}
%
können dieses Paket nicht verwenden und müssen auf die durch
\texttt{hyph-utf8} bereitgestellten Muster zurückgreifen.
...
\item\label{enum:tradnstd} Sondertrennungen (\emph{engl.:} non-standard
  hyphenation), das heißt Trennungen, die nach speziellen Regeln
  erfolgen, wie die \emph{ck}- oder die Dreikonsonantenregel, kann
  \TeX82 nicht automatisch behandeln.  Aus diesem Grund sind solche
  Trennstellen in diesen Trennmustern nicht berücksichtigt.  Das Paket
  \texttt{Babel} stellt jedoch verschiedene Kürzel zur Verfügung, mit
  denen solche Trennstellen im Quelldokument ausgezeichnet werden
  können.%
  \footnote{Sondertrennungen werden in \TeX\ mit Hilfe des Kommandos
  ...
  Beachte, im letzten Beispiel sind die Trennstellen nach der
  Auszeichnung verloren gegangen.  Es handelt sich dabei um einen
...
%
  soll in einer zukünftigen Version Sondertrennungen automatisch
  erkennen und richtig trennen können.  Eine physische Auszeichnung im
  Quelltext ist dann nicht mehr erforderlich.  Die entsprechenden
  Sondertrennmuster werden ebenfalls im Rahmen dieses Projekts erstellt.
...
Die aktuelle Ausgabe der Trennmuster ist im Dateibereich unter der
Projekt-\Abk{url}%
\footnote{\url{http://groups.google.de/group/trennmuster-opensource?hl=de}}
%
oder im \Abk{ctan} erhältlich.  Im Repositorium befindet sich ein

...
\hyperref[enum:tradDoppeld]{Regeln~\textit{T\ref{enum:tradDoppeld}}}
und~\hyperref[enum:reformDoppeld]{\textit{R\ref{enum:reformDoppeld}}}).
...
\item\label{enum:tradDoppeld} In mehrdeutigen Wörtern werden nur
  ...
  \hyperref[enum:tradSinn]{Regel~\textit{T\ref{enum:tradSinn}}} und ist
  nicht als übereinstimmende Trennstelle anzusehen.
...
\item\label{enum:reformDoppeld} In mehrdeutigen Wörtern werden nur
  ...
  Beachte, die Trennstelle ">Drucker-zeugnis"< ist in der Interpretation
  \emph{Druck-Erzeugnis} irreführend.  Sie entfällt nach
  \hyperref[enum:tradSinn]{Regel~\textit{R\ref{enum:reformSinn}}} und
  ist nicht als übereinstimmende Trennstelle anzusehen.
...
%
Da dieses Repositorium eine große Textdatei enthält, sollte nach dem
Klonen für einen reibungslosen Betrieb die Konfigurationsvariable
\verb+core.DeltaBaseCacheLimit+ vom Standardwert 16\,MB auf einen
höheren Wert gesetzt werden.  Dies kann mit den folgenden Kommandos
erreicht werden:
\begin{lstlisting}[style=shell]
...
\Programm{Git} enthält in der Version 1.5.6.4 einen Patch, der ein
Problem mit dem Netzwerkzugriff auf dieses Repositorium behebt.  Mit
älteren \Programm{Git}-Versionen können auch höhere Werte für
\verb+core.DeltaBaseCacheLimit+ sinnvoll sein.
...
\newcommand{\antwort}[1]{%
  \medskip\par
  {\noindent\fragefont\ref{#1}.~\csname #1\endcsname\par}
...
\frage{faInfo}{Wie kann ich mich über dieses Projekt informieren?}

\frage{faHelfen}{Wie kann ich helfen?}

\frage{faDateien}{Welche Dateien sollte ich mir ansehen?}

\frage{faVorschlaege}{Sollten Trennfehler einzeln oder gesammelt
  eingereicht werden?}

\end{enumerate}


\antwort{faInfo}{Zentrale Anlaufstelle ist die Projekt-\Abk{url}.%
\footnote{\url{http://groups.google.de/group/trennmuster-opensource?hl=de}}
%
Nach Anmeldung%
\footnote{Falls Sie die Anmeldung bei \Programm{Google} scheuen, können
Sie uns vielleicht beim Umzug auf einen anderen Host helfen?  Die
Anforderungen an einen alternativen Projekthost können auch privat oder
in \Datei{de.comp.text.tex} besprochen werden.}
%
kann die Mailingliste für Fragen und Hinweise genutzt werden, 
ersatzweise auch die Newsgruppe \Datei{de.comp.text.tex}.}


\antwort{faHelfen}{Es gibt mehrere Möglichkeiten:
\begin{itemize}
...
\item Besonders hilfreich wäre Mitarbeit am Projekt.  Obwohl schon
  ...
  zugrundeliegt, von möglichst vielen Menschen in verteilter Arbeit auf
  Richtigkeit zu prüfen.

  Bis dahin bleiben noch viele offene Fragen zu klären und viel zu tun.
  Bei Interesse ist es am einfachsten, Fragen, Hinweise und Ideen an die
  Mailingliste zu schicken.  Als Einstieg kann die (grobe) Aufgabenliste
  in der Projektbeschreibung dienen.
...
\item Die Datei \Datei{BUGS} beschreibt ein paar bekannte systematische
  Fehler der Trennmuster.

\item Das Entwicklerrepositorium enthält in der Datei \Datei{dateikopf}
  eine Beschreibung des Formats der Wortliste.

\item Die \Abk{html}-Präsenz des Projekts enthält einen Dateibereich mit
  weiteren Dateien, etwa einem Archiv der Diskussion im
  \Abk{mbox}-Format.
\end{itemize}
}


\antwort{faVorschlaege}{Das ist egal.  Es sollten allerdings die
folgenden Hinweise beim Einreichen von Fehlern beachtet werden:

\begin{description}\setkomafont{descriptionlabel}{\normalfont\itshape}
\item[Aktualität] Wenn Sie ältere Trennmusterdateien verwenden, prüfen
  Sie zunächst, ob der Fehler auch mit aktuellen Trennmustern auftritt.
  Aktuelle Trennmuster sind im \Abk{ctan} im Paket \texttt{dehyph-exptl}
  oder im Dateibereich unter der Projekt-\Abk{url} erhältlich.

\item[Informationen] In einem Fehlerbericht sollte für das betreffende
  Wort die richtige und bei einzelnen Worteinreichungen auch die falsche
  Trennung angegeben werden.  Außerdem die genaue Version der
...
\item[Listenformat] Es erleichtert die Korrektur, wenn
  ...
  \item Ab der vierten Spalte \emph{können} weitere Trennvarianten
    folgen, etwa die falsche Trennung.  Eine kurze Erklärung sollte dann
    darauf eingehen.
...
\item[Betreff] Der Betreff einer Fehlermeldung sollte aussagekräftig
  ...
  Zuordnung das entsprechende Kriterium aus \autoref{sec:fehltrennungen}
  dem Betreff der E-Mail in eckigen Klammern vorangestellt werden, zum
  Beispiel:
\begin{lstlisting}[style=Text]
Betreff: [C] sonnendurchfluteten
\end{lstlisting}
  für eine Fehltrennung des Wortes \emph{sonnendurchfluteten}, die durch
  einen Fehler in der Wortliste hervorgerufen wird.
\end{description}
}

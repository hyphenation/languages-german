...
\item\label{enum:tradVersal} Wörter mit \emph{ß} werden in der
  ...
  Beachte, tritt durch den Ersatz des~\emph{ß} durch~\emph{SS} im
  Versalsatz eine Mehrdeutigkeit auf, so wird zugunsten der normalen
  Schreibweise getrennt (siehe auch
  \regelref{enum:tradDoppeld}):\par\nobreak

  \begin{tabular}[t]{TTUU}
    \textls{MAS-SE} & (Mas-se) & \textls{MA-SSE} & \textls{MASSE}\\
    \textls{FLÖS-SE} & (flös-se) & \textls{FLÖ-SSE} & \textls{FLÖSSE}\\
  \end{tabular}

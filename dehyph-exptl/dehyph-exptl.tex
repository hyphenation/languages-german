...
\item\label{enum:tradEszett} Wird der Buchstabe~\emph{ß} durch
  ...
  \item Wenn durch den Ersatz von~\emph{ß} an dieser Stelle keine
    ...
    durch~\emph{SS} ersetzt, so bleibt~\emph{SS} stets ungetrennt.  Die
    Trennung richtet sich dann nach der Schreibweise mit~\emph{ß} im
    Quelldokument.

    Beachte, existiert ein Wort in verschiedenen Varietäten in der
    Schreibweise mit~\emph{ß} und mit~\emph{ss}, so wird aufgrund dieser
    Einschränkung \emph{s-s} stets getrennt:
    ...
    Beachte, wenn durch den Ersatz des~\emph{ß} an entfernten Stellen
    keine eindeutige Trennung möglich wird, zum Beispiel an Wortfugen,
    so werden die betroffenen Trennungen gemäß

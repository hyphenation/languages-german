...
\item\label{enum:tradVersal} Wörter mit \emph{ß} werden in der
  ...
  \begin{itemize}
  \item Wenn durch das Ersetzen von~\emph{ß} durch~\emph{SS} im
    Versalsatz eine Mehrdeutigkeit auftritt, so wird zugunsten der
    Bedeutung des Wortes in der normalen Schreibweise getrennt (siehe
    auch \regelref{enum:tradDoppeld}).
  \end{itemize}
  \nobreak
  ...
  \lstinline[style=LaTeX]+\MakeUppercase+ durch~\emph{SS} ersetzen, so

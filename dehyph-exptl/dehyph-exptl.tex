...
\newcommand*{\trennung}[2]{%
   \makebox[0pt][l]{%
     \color{#1}%
     \smash{\rule[-2.5pt]{\widthof{#2}}{.7pt}}%
   }%
   #2%
}
...
\item Die Qualität der Trennung zusammengesetzter Wörter ist deutlich
  geringer als die der Trennung einfacher Wörter.
...
%
  ...
  \emph{S-S} getrennt: \textls{">GR\ftr{ÖS-S}E"<},
  \textls{">GR\ftr{ÜS-S}E"<}, \textls{">M\ftr{AS-S}ES"<}.

\item Abweichende Schreibweisen in der traditionellen Rechtschreibung,
  die in Österreich und der Schweiz verwendet werden, werden von den
  herkömmlichen Trennmustern häufig falsch oder unzureichend getrennt:
  ">Gro\ftr{s-so}n-kel"<, ">Ku\ftr{s-sh}and"<, ">Ma\ftr{ssn}ah-me"<,
  ">mi\ftr{s-sa}ch-ten"< (nur Schweiz) und ">Ex-pre\ftr{ssz}ug"<,
  ">Fit-ne\ftr{s-sc}en-ter"<, ">Fit-ne\ftr{sst}rai-ner"<.

\item Die herkömmlichen Trennmusterdateien enthalten eine Mischung aus
  \Abk{t1}- und \Abk{ot1}-kodierten Mustern.  Letztere sind zudem
  unvollständig.  Mit Erscheinen von 16-Bit-fähigen \TeX-Varianten
  werden saubere Trennmuster nötig.
\end{enumerate}
...
\autoref{sec:installation} und die Datei \Datei{INSTALL}.
...
\emph{Hinweise:} Beziehen sich die Konventionen für die reformierte
Rechtschreibung auf die traditionelle Rechtschreibung, so werden die
entsprechenden Regeln etwas ausführlicher dargestellt.  Die folgenden
Abschnitte enthalten jedoch keine vollständige Aufstellung der
Silbentrennregeln.  Diese sind den entsprechenden Regelwerken zu
entnehmen.

Werden in Einzelfällen von den hier gezeigten Konventionen abweichende
Trennungen gewünscht, so kann die Trennung mit den \TeX- und 
...
In Liangs Trennalgorithmus werden Groß- und
...
\item\label{enum:tradDoppeld} In mehrdeutigen Wörtern werden nur
  übereinstimmende Trennstellen zugelassen.  Es gelten die folgenden
  Einschränkungen:

  \begin{itemize}
  \item Bei mehrdeutigen Wörtern endend auf \emph{-ende, -enden, -endes}
    wird stets die Trennung der Partizipform des Verbs verwendet.
...
\item\label{enum:reformDoppeld} In mehrdeutigen Wörtern werden nur
  übereinstimmende Trennstellen zugelassen.  Es gilt die folgende
  Einschränkung:

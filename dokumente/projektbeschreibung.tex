...
\item[Sprachanbindung] Die Trennmuster für die (traditionelle)
  (deutsch)schweizerische Rechtschreibung benötigen eine Sprachanbindung
  für \texttt{Babel} und \texttt{Polyglossia}.
...
\item[mehrere Trennmuster laden] Für einzelne Dokumente ist es
  ...
  vorhanden.  Es wäre daher sinnvoll, Trennmuster auch dynamisch während
  der Ausführung laden zu können.  Wie wird mit kollidierenden Mustern
...
\item[Verallgemeinerung des Trennalgorithmus] Im Satz mit gebrochenen
  Schriften sollen langes und rundes~s ohne Auszeichnung im Quelltext
  richtig behandelt werden.  Vorstellbar sind eigene Muster zur
  Erkennung von Rund-s (und falschen Ligaturen -- oder besser den
  richtigen?) und ein Durchsuchen mehrerer entsprechender Tries während
  des Absatzumbruchs.  $\Rightarrow$ Vorschlag für Lua\TeX-Entwickler
  ausarbeiten
